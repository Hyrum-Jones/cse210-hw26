public class Car
{
    ...
    public Door trunkDoor = new Door();
    ...
    public void Run()
    {
        ...
        while (IsRunning)
        {
            ...
            if (clicker.IsArmed)
            {
                if (driverDoor.IsOpen || passengerDoor.IsOpen || trunkDoor.IsOpen)
                {
                    carHorn.Honk();
                }
            }
            ...
            else if (input == "3") trunkDoor.Toggle();
            else if (input == "4") clicker.IsArmed = true;
            else if (input == "5") clicker.IsArmed = false;
            ...
            Console.WriteLine($"DOORS:         [Driver: {driverDoor.GetStatus()}] [Passenger: {passengerDoor.GetStatus()}] [Trunk: {trunkDoor.GetStatus()}]");
            Console.WriteLine("----------------------------------");
        }
    ...
    public void PrintMenu()
    {
        ...
        Console.WriteLine("3. Open/Close Trunk Door");
        Console.WriteLine("4. Set Alarm (Arm)");
        Console.WriteLine("5. Disable Alarm (Disarm)");
